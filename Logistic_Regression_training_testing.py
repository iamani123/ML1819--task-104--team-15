...
for k in range(1,10):
        acc[0,n]= (k/10)*100
        X_train, X_test, y_train,y_test  = train_test_split(X,y,test_size=(1-k/10), random_state=0)
        ...
        print("train_size",(k/10)*100," ",result)
        acc[1,n]=result;
        n=n+1
...
ax.set_xlabel('% of Training Data', fontsize=5)
ax.set_ylabel('Accuracy', fontsize=5)
ax.legend(loc=2)
fig.savefig('GraphforLogisticRegression_train.png')